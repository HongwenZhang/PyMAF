        ...
        if checkpoint_file is not None:
            ...
            for model in self.models_dict:
                if model in checkpoint:
                    ...
                    print(f'Checkpoint {model} loaded from: {checkpoint_file}')

    def load_backbone(self, checkpoint_file=None):
        ...
        if checkpoint_file is not None:
            ...
            for model in self.models_dict:
                if model in checkpoint:
                    backbone_state_dict = {}
                    for name, m in checkpoint['model'].items():
                        if name.split('.')[0] == 'encoders':
                            backbone_state_dict[name] = m
                            print('name', name)
                    self.models_dict[model].load_state_dict(backbone_state_dict, strict=False)
                    print(f'Backbone of Checkpoint {model} loaded from: {checkpoint_file}')
